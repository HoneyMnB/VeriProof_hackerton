...
각 변수의 기본값은 `1`이다. 이 변수에는 보안 비밀 값이 아니라 Secret Manager의
숫자형 버전 번호만 저장한다.
...
삭제하지 말고 새 CryptoKeyVersion을 만든 뒤 primary version으로 전환한다.

애플리케이션에는 CryptoKey 리소스 이름을 설정한다. 애플리케이션은 실행 시 현재
primary version을 조회하므로 키 버전 교체를 위해 코드를 수정할 필요는 없다.

키 교체 후 다음 항목을 확인한다.

1. Cloud Run 서비스 계정이 새 primary version으로 서명할 수 있는가
...
Manager 버전, KMS primary version 및 Cloud Run revision이 서로 맞는 상태인지 함께
확인해 복구한다.
