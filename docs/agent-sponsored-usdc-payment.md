...
2. API는 서버 설정의 service user와 Buyer KMS 공개키를 사용한다. 요청이 보낸 지갑이나 사용자는 받지 않는다.
...
AGENT_SPONSORED_PAYMENT_BUYER_USERNAME=<사전에 생성한 service user username>
...
일치해야 한다. API service user는 사전에 생성하고 활성 상태여야 하며, 이 계정에
발급된 라이선스가 Agent 구매의 소유권 기록이다.
...
- API는 bearer token을 상수 시간 비교하고, Buyer wallet·license user를 환경 설정에서만 결정한다.
...
- `503`: sponsor/KMS/RPC/settlement 또는 Agent principal 설정을 사용할 수 없음

## 배포 및 검증

1. migration `ip.0021_sponsoredpaymentintent_channel`까지 적용한다.
2. Agent service user, Buyer KMS 키, sponsor KMS 키, 공통 bearer token을 설정한다.
