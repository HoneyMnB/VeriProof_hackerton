...
(function (global) {
    ...
    SalesDashboard.prototype.toggleDetail = function (row, sale, button) {
        ...
        [["dashboard.table.buyer", sale.buyer_wallet, ""], ["dashboard.license_id", sale.license_id, ""], ["dashboard.payment_id", sale.payment_tx_sig || "—", "is-wide", true], ["dashboard.certificate_id", sale.certificate_tx_sig || "—", "is-wide"]].forEach(function (entry) { var item = document.createElement("div"); var term = document.createElement("dt"); var value = document.createElement("dd"); item.className = "vp-sales-inline-detail__item " + entry[2]; term.textContent = t(entry[0]); value.textContent = entry[1]; if (entry[3] && sale.payment_tx_sig) { var link = document.createElement("a"); link.className = "vp-sales-inline-detail__explorer-link"; link.href = "https://explorer.solana.com/tx/" + encodeURIComponent(sale.payment_tx_sig) + "?cluster=devnet"; link.target = "_blank"; link.rel = "noopener noreferrer"; link.textContent = t("dashboard.view_transaction"); value.append(" ", link); } item.append(term, value); list.appendChild(item); });
